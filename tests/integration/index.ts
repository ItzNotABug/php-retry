...
import { runCommentTests } from './comments';
...
function logSummary(
  phpunitResults: ScenarioResult[],
  commentResults: ScenarioResult[],
  durationMs: number,
): void {
  if (verbose) {
  ...
  const totalTests = phpunitResults.reduce((sum, r) => sum + r.retryCount, 0);
  const totalScenarios = phpunitResults.length + commentResults.length;
  const totalExpected = scenarios.length + 7; // 7 comment tests

  console.log('Summary');
  console.log('-------');
  console.log(
    `  PHPUnit scenarios: ${phpunitResults.length}/${scenarios.length} passed`,
  );
  console.log(`  Comment tests: ${commentResults.length}/7 passed`);
  console.log(`  Total: ${totalScenarios}/${totalExpected} passed`);
...
async function runIntegrationTests(): Promise<void> {
  ...
  ensureFileExists(
    distEntry,
    'dist/index.js not found; run bun run build first',
  );
  await runPrechecks();
  ...
  try {
    const phpunitResults = await runScenarios(nodePath, tmpDir);

    // Run comment tests (don't require Docker)
    if (!verbose) {
      console.log('Comment integration tests');
      console.log('-------------------------');
      console.log('');
    }
    const commentResults = await runCommentTests(verbose);
    if (!verbose) {
    ...
    logSummary(phpunitResults, commentResults, totalDuration);
  } finally {
    await dockerComposeDown();
    cleanupFiles(tmpDir);
  }
